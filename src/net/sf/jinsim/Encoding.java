...
public class Encoding {
	...
	public static String decodeString(byte[] byteArray) {
		if (new String(byteArray).startsWith("^S")) {
...
//		}
		...
		ByteArrayOutputStream byteArrayStream = new ByteArrayOutputStream(text
				.length() * 2);

		char[] currentCodePage = codePageLatinToUnicode;

		char[][] codePages = { codePageLatinToUnicode,
				codePageCentralEuropeToUnicode, codePageCyrillicToUnicode,
				codePageGreekToUnicode, codePageTurkishToUnicode,
				codePageBalticToUnicode, codePageJapaneseToUnicode };
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (ch > 127) {
				...
				if (!found) {
					...
					try {
						byteArrayStream.write(String.valueOf(ch).getBytes());
					} catch (IOException e) {
